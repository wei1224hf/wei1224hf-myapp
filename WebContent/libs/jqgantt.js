        ...
        */
        var core = {
            ...
            */
            create: function (element) {
                ...
                */
                if (typeof settings.source !== "string") {
                    element.data = settings.source.Rows;
                    element.rowsNum = settings.source.Total;
                    element.pageCount = Math.ceil(element.rowsNum / settings.itemsPerPage);  
                    
                    core.init(element);
                } else {
                	core.readServer(element,0);
                	/*
            ...
            readServer: function(element,page){
            	var theData = settings.serverData;
            	if(page==0){
            		theData.page = 1;
            	}else{
            		theData.page += page;
        		...
        		$.ajax({
        			...
        			,success : function(response) {
        				element.data = response.Rows;
                        element.rowsNum = response.Total;
                        element.pageCount = Math.ceil(element.rowsNum / settings.itemsPerPage);  
                        core.init(element);
        			}
        		});
            ...
            leftPanel: function (element) {
                ...
                var entries = [];
                var checkbox_ = "";
                if(this.settings.checkbox) checkbox_ = '<input type="checkbox" />';
                $.each(element.data, function (i, entry) {
                	...
                	if(2>1){
                        ...
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">'+checkbox_ + entry.name + '</span>');
                        entries.push('</div>');
            ...
            createProgressBar: function (days, cls, desc, label, dataObj) {
                ...
                var bar = $('<div class="bar"><div class="fn-label" id="cell_'+dataObj.id+'">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
        ...
        */
        var tools = {
            ...
            dateDeserialize: function (dateStr) {
                ...
                var date2 = new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes());
                return date2;
            },
